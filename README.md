...
Valor writes to Celo and Avalanche mainnet constantly — every fight, rank-up, reward, purchase, transfer and re-arm is a real transaction. Transactions are a consequence of players doing things, not a target: the backend pays gas so a player never has to, which means every write has to earn its cost.
