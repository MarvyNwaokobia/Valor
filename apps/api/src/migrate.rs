...
const MIGRATIONS: &[(&str, &str)] = &[
    ...
    // Standalone table with no FK to players (a verify-step escalation happens before a
    // player row exists), so it is order-independent — filed alphabetically.
    ("add_agent_escalations.sql", include_str!("../migrations/add_agent_escalations.sql")),
